function insertName() {
    firebase.auth().onAuthStateChanged(user => {
        ...
        if (user) {
            ...
            //document.getElementById("user-name").innerText = user_Name;    //using javascript
            //method #2:  insert using jquery
            $("#user-name").text(user_Name); //using jquery
...
function readQuote() {
    db.collection("quotes").doc("tuesday")
        .onSnapshot(tuesdayDoc => {
            console.log("current doc data: ", tuesdayDoc.data());
            document.getElementById("quote-goes-here").innerHTML = tuesdayDoc.data().quote;

            // with jquery
            // $("#quote-goes-here").text(tuesdayDoc.data().quote);
            // $("#quote-goes-here").text(tuesdayDoc.data()[quote]);
        })
}
readQuote();
